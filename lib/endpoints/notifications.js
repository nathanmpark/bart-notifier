...
var notificationEntity = {
  ...
  get: {
    ...
    service: function(req) {
      ...
      var user = self.endpoint.service._getUser(req)
      var notifications_col = self.endpoint.service.db.getCollection(self.endpoint.service.cols.notifications)
  ...
  patch: {
    ...
    service: function(req, res) {
      ...
      var user = self.endpoint.service._getUser(req)
      var col = self.endpoint.service.db.getCollection(
        self.endpoint.service.cols.notifications)
      ...
      if (notification === null) {
        throw new self.endpoint.service.errors.NotFound()
      }
      ...
      var scol = self.endpoint.service.db.getCollection(self.endpoint.service.cols.stations)

      if (req.body.station) {
        if (scol.findOne({'_id': req.body.station.toUpperCase()}) == null) {
          throw new self.endpoint.service.errors.BadRequest('unknown station: ' + req.body.station)
        }
...
var notificationList = {
  ...
  post: {
    ...
    service: function(req, res) {
      ...
      var col = self.endpoint.service.db.getCollection(self.endpoint.service.cols.stations)
      if (col.findOne({'_id': req.body.station.toUpperCase()}) == null) {
        throw new self.endpoint.service.errors.BadRequest('unknown station: ' + req.body.station)
      }
      // FIXME: this test should be tighter
      if (col.findOne({'_id': req.body.destination.toUpperCase()}) == null) {
        throw new self.endpoint.service.errors.BadRequest('unknown destination: ' + req.body.destination)
      }
      var delta = req.body.delta * 60 * 1000
      var dt = Date.parse(req.body.datetime)
      if (!checkNotificationTime(dt, delta)) {
        throw new self.endpoint.service.errors.BadRequest('bad datetime: ' + req.body.datetime)
      }

      var col = self.endpoint.service.db.getCollection(self.endpoint.service.cols.notifications)
