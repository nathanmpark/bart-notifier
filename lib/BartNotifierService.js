...
module.exports = { 
  ...
  _main: function(options) {
    // XXX leaving _main until there's a better way to execute code prior to
    //     app initialization/server start
    var self = this

    try {
      if (self.seedDb || self.clearDb) {
    ...
    carbond.ObjectServer.prototype._main.call(self, options)
  },
  ...
  _seedDb: function() {
    ...
    try {
      stations = JSON.parse(
                  fs.readFileSync(
                    path.join(__dirname, 'fixtures', 'stations.json'), 'utf8'))
    }
    catch (e) {
    ...
    try {
      users = JSON.parse(
                fs.readFileSync(
                  path.join(__dirname, 'fixtures', 'users.json'), 'utf8'))
    }
    catch (e) {
